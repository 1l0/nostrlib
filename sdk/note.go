...
func (sys *System) PrepareNoteEvent(ctx context.Context, evt *nostr.Event) (targetRelays []string) {
	...
	evt.Content = domainRe.ReplaceAllStringFunc(evt.Content, func(s string) string {
	...
	evt.Content = nostrRe.ReplaceAllStringFunc(evt.Content, func(s string) string {
		if strings.HasPrefix(s, "nostr:") {
			return s
		}
		if _, _, err := nip19.Decode(s); err == nil {
			return "nostr:" + s
		}
		return s
	})
	...
	for block := range nip27.Parse(evt.Content) {
		...
		case nostr.EventPointer:
			...
			var relay string
			if pk == nostr.ZeroPK {
				// we actually try to fetch the note to learn its author
				ctx, cancel := context.WithTimeout(ctx, time.Second*1)
				evt, rl, err := sys.FetchSpecificEvent(ctx, b, FetchSpecificEventParameters{WithRelays: false})
				cancel()
				if err == nil {
					pk = evt.PubKey
					if len(rl) > 0 {
		...
		// if that block has a pubkey associated with it we'll have to publish to those relays
		if pk != nostr.ZeroPK {
			targetRelays = nostr.AppendUnique(targetRelays, sys.FetchInboxRelays(ctx, pk, 4)...)
		}
	}

	// iterate through the content and add hashtags to event tags
	for _, tag := range tagRe.FindAllString(evt.Content, -1) {
		spl := strings.Split(tag, "#")
		tagValue := spl[len(spl)-1]
		if evt.Tags.FindWithValue("t", tagValue) == nil {
			evt.Tags = append(evt.Tags, nostr.Tag{"t", tagValue})
		}
	}
